...
void	redraw_player(t_data *data)
{
	...
	i = get_tile_index(current);
	if (i)
		draw_tile(data, i, px, py);
	mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
		data->frames[data->direction * 3 + data->frames], data->smooth_x,
		data->smooth_y);
}
...
void	update_var(t_data *data, int nx, int ny)
{
...
void	move_player(t_data *data, int x, int y, int dir)
{
	int	new_posx;
	int	new_posy;
